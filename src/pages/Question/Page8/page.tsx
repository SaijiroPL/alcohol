...
export default function({
  ...
  function onNext() {
    const key = saveStore()
    history.push(`/question/9?key=${key}`);
  }
  function onBack() {
    const key = queryString.parse(window.location.search).key?.toString()
  ...
  return (
    <div className='ac-question-container'>
      ...
      <div className='ac-question-content'>
        <div className='ac-question-text'>{PAGE_INFOES[7].title}</div>
      </div>
      <MultiChoice answer={answer} setAnswer={setAnswer} options={PAGE_INFOES[7].selections} elementStyle={{ margin: '10px' }} columns={1}/>
